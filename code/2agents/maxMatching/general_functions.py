...
def create_G(c, utilities, s, point=(1, 0), n=2):
    """
    ...
    for x in range(n):
        for i in range(num_agents_copies):
            ...
            for j in range(num_chores):
                if j < m:  # real chore
                    chore = 't_' + str(c) + '_' + str(j)
                    weight = utilities[j][x] * point[x]
                else:  # dummy chore
                    chore = 'd_' + str(c) + '_' + str(j-m)
                    weight = 0
                if x == 0 and i == 0:
                    chores.append(chore)
                G.add_edge(agent, chore, weight=weight)
                # print("(", agent, ",", chore, ") added with weight ", weight)
                count += 1
    # print(count)
    return G, chores


def recognize_agent_and_chore(match):
    if match[0][0] == 'A':
        return 1, match[1]
    elif match[1][0] == 'A':
        return 1, match[0]
...
def get_partial_divisions(matching):
    ...
    for match in matching:
        agent, chore = recognize_agent_and_chore(match)
    ...
    for c in range(len(A)):
        ...
        for chore in A_c:
            if chore[0] == 'd':  # a dummy chore
                ...
                u2.append(0)
                continue
            splitted = chore.split('_')
            # print("splitted: ", splitted)
            chore_indx = int(splitted[-1])
            # add chore's utility
            u1.append(utilities[c][chore_indx][0])
            u2.append(utilities[c][chore_indx][1])
    return u1, u2
...
def divideToGroups(G, chores):
    ...
    for chore in chores:  # include dummy chores
        # print("edge: ", G.get_edge_data('t_1_0', 'A0'))
